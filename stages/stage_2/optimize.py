...
CONS_Q_A_OPTIMIZER_PATTERN = assemble("((c (quote (: . sexp)) (: . args)))")
...
VAR_CHANGE_OPTIMIZER_CONS_EVAL_PATTERN = assemble("((c (quote (: . sexp)) (: . args)))")


def var_change_optimizer_cons_eval(r, eval):
    """
    This applies the transform
    ((c (quote (op SEXP1...)) (ARGS))) => (quote RET_VAL) where ARGS != @
    via
